...
function crossSafeAiErrorMessage(err) {
  return err?.message || "CrossSafe AI is temporarily unavailable.";
}
...
(function(){
  ...
  form.onsubmit=async e=>{e.preventDefault();const q=input.value.trim();if(!q)return;add(q,'user');input.value='';const typing=add('Thinking…','bot typing');try{const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:q,context:context()})});const data=await r.json();typing.remove();if(!r.ok)throw new Error(data.error||'Request failed');add(data.reply||'I could not generate a response.','bot')}catch(err){typing.remove();add('I couldn't connect to CrossSafe AI right now. Please check the Vercel deployment and GEMINI_API_KEY.','bot')}};
})();
