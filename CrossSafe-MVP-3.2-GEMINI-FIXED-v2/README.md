...
The AI chat uses the Vercel serverless endpoint `/api/chat` and Google Gemini.
...
Do not put the real API key in frontend JavaScript or commit it to GitHub.

After adding or changing environment variables, **Redeploy** the latest deployment.

If the AI still fails, the chat UI now reports the API error returned by `/api/chat`, which makes configuration/model errors easier to diagnose.
