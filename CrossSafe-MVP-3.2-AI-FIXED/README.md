...
The OpenAI API key must NOT be placed in frontend JavaScript or committed into this ZIP/Git repository.

Because an API key was pasted into chat, treat that key as exposed and **revoke/rotate it in OpenAI Platform before use**. Create a new secret key, then configure it as the `OPENAI_API_KEY` environment variable on your deployment platform.
...
3. Optionally add `OPENAI_MODEL=gpt-5.6-luna`.
4. Redeploy.
